...
int main(){
    ...
    for (int n=0; n < N; n++){ //itera sui blocchi
        ...
        for(int l=0; l < L; l++){ //itera sui lanci per blocco
            ...
            S=get_S_T(S_0,T,R,r,sigma,&rnd); // con discretizzazione (R sottointervalli)
            block_sum_call_dis+=get_price(call,S,r,K,T);
            block_sum_put_dis+=get_price(put,S,r,K,T);
        ...
        if (n==0){ //pone artificialmente a zero l'errore relativo alla media di un blocco solo (evitando Nan negli output)
            error_call_dir=0;
            error_put_dir=0;
            error_call_dis=0;
            error_put_dis=0;
            ...
            //calcola la deviazione standard cumulativa al blocco n
