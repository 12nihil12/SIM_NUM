...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ifstream cinf;
    cinf.open(string(OUTPUT_DIR) + "CONFIG/conf-1.xyz"); //per invertire le velocità, mette le ultime coordinate al posto delle penultime 
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
...
void System :: read_configuration(){
  ifstream cinf;
  if (_restart){
    cinf.open(string(OUTPUT_DIR) + "CONFIG/config.xyz");//per invertire le velocità mette le penultime coordinate al posto delle ultime
