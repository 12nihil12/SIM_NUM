...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ifstream cinf;
    cinf.open(string(OUTPUT_DIR) + "CONFIG/conf-1.xyz"); 
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
    ...
    sumv.zeros();
    for (int i=0; i<_npart; i++){
      which=floor(_rnd.Rannyu(0,6)); 
      switch(which) {
        case 0: vx(i)=  v_T; break;
        case 1: vx(i)= -v_T; break;
        case 2: vy(i)=  v_T; break;
        case 3: vy(i)= -v_T; break;
        case 4: vz(i)=  v_T; break;
        case 5: vz(i)= -v_T; break;
      }
      sumv(0)+=vx(i);
      sumv(1)+=vy(i);
      sumv(2)+=vz(i);
