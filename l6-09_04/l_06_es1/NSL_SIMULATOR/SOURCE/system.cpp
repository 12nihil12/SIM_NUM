...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream Primes(string(INPUT_DIR) + "Primes");
  Primes >> p1 >> p2 ;
  Primes.close();
  int seed[4]; // Read the seed of the RNG
  ifstream Seed(string(INPUT_DIR) + "seed.in");
  Seed >> seed[0] >> seed[1] >> seed[2] >> seed[3];
  _rnd.SetRandom(seed,p1,p2);

  ofstream couta(string(OUTPUT_DIR) + "acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input(string(INPUT_DIR) + "input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
  coutf.open(string(OUTPUT_DIR) + "output.dat");
  string property;
  double delta;
  while ( !input.eof() ){
    cout << "hi"<<endl;
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      for(int i=0; i<_ndim; i++){
        coutf << setw(12) << _side[i];50
#Comment!
LJ  -0.490000    0.000000    0.000000
LJ  -0.470000    0.000000    0.000000
LJ  -0.450000    0.000000    0.000000
LJ  -0.430000    0.000000    0.000000
LJ  -0.410000    0.000000    0.000000
LJ  -0.390000    0.000000    0.000000
LJ  -0.370000    0.000000    0.000000
LJ  -0.350000    0.000000    0.000000
LJ  -0.330000    0.000000    0.000000
LJ  -0.310000    0.000000    0.000000
LJ  -0.290000    0.000000    0.000000
LJ  -0.270000    0.000000    0.000000
LJ  -0.250000    0.000000    0.000000
LJ  -0.230000    0.000000    0.000000
LJ  -0.210000    0.000000    0.000000
LJ  -0.190000    0.000000    0.000000
LJ  -0.170000    0.000000    0.000000
LJ  -0.150000    0.000000    0.000000
LJ  -0.130000    0.000000    0.000000
LJ  -0.110000    0.000000    0.000000
LJ  -0.090000    0.000000    0.000000
LJ  -0.070000    0.000000    0.000000
LJ  -0.050000    0.000000    0.000000
LJ  -0.030000    0.000000    0.000000
LJ  -0.010000    0.000000    0.000000
LJ   0.010000    0.000000    0.000000
LJ   0.030000    0.000000    0.000000
LJ   0.050000    0.000000    0.000000
LJ   0.070000    0.000000    0.000000
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ifstream cinf;
    cinf.open(string(INPUT_DIR) + "CONFIG/conf-1.xyz");
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  ifstream input(string(INPUT_DIR) + "properties.dat");
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp(string(OUTPUT_DIR) + "potential_energy.dat");
        ...
        ofstream coutk(string(OUTPUT_DIR) + "kinetic_energy.dat");
        ...
        ofstream coutt(string(OUTPUT_DIR) + "total_energy.dat");
        ...
        ofstream coutte(string(OUTPUT_DIR) + "temperature.dat");
        ...
        ofstream coutpr(string(OUTPUT_DIR) + "pressure.dat");
        ...
        ofstream coutgr(string(OUTPUT_DIR) + "gofr.dat");
...
LJ   0.490000    0.000000    0.000000
        ...
        ofstream coutpv(string(OUTPUT_DIR) + "pofv.dat");
        ...
        ofstream coutpr(string(OUTPUT_DIR) + "magnetization.dat");
        ...
        ofstream coutpr(string(OUTPUT_DIR) + "specific_heat.dat");
        ...
        ofstream coutpr(string(OUTPUT_DIR) + "susceptibility.dat");
        ...
        ofstream coutf;
        coutf.open(string(OUTPUT_DIR) + "output.dat",ios::app);
...
void System :: finalize(){
  ...
  /*
  if(_measure_tenergy )
  coutf.open(../CROSS_SIM_DATA + "cross_sim_tot_energy.dat",ios::app);
  coutf << "#SIM  TE_AVE  ERROR" << endl; 
  coutf.open(../CROSS_SIM_DATA + "cross_sim_tot_energy.dat",ios::app);
  coutf << "#SIM  TE_AVE  ERROR" << endl; 
    */
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(string(OUTPUT_DIR) + "CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.close();
    coutf.open(string(OUTPUT_DIR) + "CONFIG/conf-1.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open(string(OUTPUT_DIR) + "CONFIG/config.spin");
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open(string(OUTPUT_DIR) + "CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open(string(INPUT_DIR) + "CONFIG/config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
void System :: measure(){ // Measure properties
  ...
  double module_velocity;
  int bin_index;
  
  if(_measure_pofv){
    for(int i=0; i<_npart;i++){
     
      module_velocity=sqrt(dot( _particle(i).getvelocity() , _particle(i).getvelocity() ));
      if(std::isnan(module_velocity ) || std::isinf(module_velocity )){
        cout <<"Warning:ignoring particles with not valid velocity"<<endl;
        continue;
      }
      bin_index=static_cast<int>(floor(module_velocity/_bin_size_v));
      
      if(bin_index >=0 && bin_index < _n_bins_v){
  ...
  if (_measure_magnet || _measure_chi){
    for(int i=0; i<_npart; i++){
  ...
  if (_measure_chi){
    _measurement[_index_chi]=magnetization*magnetization; //moltiplicazione per b verrà fatta al termine del blocco
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv){
    _measurement[_index_cv]=tenergy_temp*tenergy_temp; //la sottrazione del valore medio quadro dell'energia, come la moltiplicazione per beta verranno fatte al termine del blocco
  }
...
void System :: averages(int blk){
  ...
  //salva le medie per cv, prima che vengano sovrascritte con altro
  double _global_av_cv=_global_av(_index_cv); 
  double _global_av2_cv=_global_av2(_index_cv); 
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(string(OUTPUT_DIR) + "kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(string(OUTPUT_DIR) + "total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(string(OUTPUT_DIR) + "temperature.dat",ios::app);
  ...
  if (_measure_pressure){
    coutf.open(string(OUTPUT_DIR) + "pressure.dat",ios::app);
  ...
  // TO BE FIXED IN EXERCISE 4
    ...
    for(int n=_index_pofv; n< _index_pofv +_n_bins_v;n++){
      ...
      coutf << setw(12) << blk
          ...
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2/****************************************************************
*****************************************************************
    _/    _/  _/_/_/  _/       Numerical Simulation Laboratory
   _/_/  _/ _/       _/       Physics Department
  _/  _/_/    _/    _/       Universita' degli Studi di Milano
...
void System :: averages(int blk){
  ...
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    coutf.open(string(OUTPUT_DIR) + "magnetization.dat",ios::app);
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
    ...
    if (_measure_cv){
    coutf.open(string(OUTPUT_DIR) + "specific_heat.dat",ios::app);
    average  = (_average(_index_cv)-pow(_average(_index_tenergy),2))/(pow(_average(_index_temp),2));
    _global_av(_index_cv)=sum_average = _global_av_cv + average;
    _global_av2(_index_cv)=sum_ave2 = _global_av2_cv + average*average;
    coutf << setw(12) << blk
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open(string(OUTPUT_DIR) + "acceptance.dat",ios::app);
