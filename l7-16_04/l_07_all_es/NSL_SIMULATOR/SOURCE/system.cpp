...
void System :: read_configuration(){
  ifstream cinf;
  if (_restart){
    cinf.open(string(OUTPUT_DIR) + "CONFIG/config.xyz");
...
void System :: measure(){ // Measure properties
  ...
  double virial=0.0;
  int bin_index; 
  if (_measure_penergy or _measure_pressure or _measure_gofr or _measure_tenergy) {
    for (int i=0; i<_npart-1; i++){
      ...
      if(isnan(dr) || isinf(dr)){
        cout <<"Warning:ignoring particle with not valid distance"<<endl;
        continue;
      }
      if(_measure_gofr){
        bin_index=static_cast<int>(floor(dr/_bin_size));
      
        if(bin_index < _n_bins){
          _measurement(_index_gofr+bin_index)+=2;
        }
  ...
  if(_measure_pofv){
    for(int i=0; i<_npart;i++){
      ...
      if(isnan(module_velocity ) || isinf(module_velocity )){
        cout <<"Warning:ignoring particle with not valid velocity"<<endl;
        continue;
      }
  ...
  // KE,TEMPERATURE, PRESSURE FOR MOLECULAR DYNAMICS
  if (_measure_kenergy or _measure_temp or _measure_pressure or _measure_tenergy && _sim_type==0){
  ...
  // KE,TEMPERATURE, PRESSURE FOR MONTE CARLO

  if(_sim_type==1){
    if (_measure_temp) _measurement(_index_temp) = _temp;
    if (_measure_kenergy) _measurement(_index_kenergy) = (3.0/2.0)*_temp;
    if (_measure_pressure) _measurement[_index_pressure] = _rho * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume; 
    if (_measure_tenergy) kenergy_temp=(3.0/2.0)*_temp; 
  }
...
void System :: averages(int blk){
  ...
  if(_measure_cv){//completa il calcolo di cv
    _average(_index_cv)  = (_average(_index_cv)-pow(_average(_index_tenergy),2))*_beta*_beta;   
  }
  _global_av  += _average;
  ...
  if (_measure_tenergy){
    ...
    coutf << setw(12) << blk
          ...
          << setw(12) << sum_average/double(blk)/_npart
          << setw(12) << this->error(sum_average, sum_ave2, blk)/_npart << endl;
    coutf.close();
  }
  ...
  // POFV //////////////////////////////////////////////////////////////////////
    ...
    for(int n=_index_pofv; n< _index_pofv +_n_bins_v;n++){
      ...
      coutf << (n-_index_pofv)*_bin_size_v
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
   ...
   if (_measure_chi){
    coutf.open(string(OUTPUT_DIR) + "susceptibility.dat",ios::app);
    average  = _average(_index_chi);
    sum_average = _global_av(_index_chi);
    sum_ave2 = _global_av2(_index_chi);
    coutf << setw(12) << blk
          << setw(12) << _beta*average/_npart
          << setw(12) << _beta*sum_average/double(blk)/_npart
          << setw(12) << _beta*this->error(sum_average, sum_ave2, blk)/_npart<< endl;
    coutf.close();    
