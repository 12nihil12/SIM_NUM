...
void Path::Printwlen() const {
    cout << left << fixed << setprecision(4)  << _len << "    | ";  
    _chromosome.raw_print();
    cout << endl;
}


void Path::Printwlen(ostream & fout) const {
    fout  << left <<fixed << setprecision(4)   << _len << "    | ";  
    _chromosome.raw_print(fout); 
    fout << endl;
}
