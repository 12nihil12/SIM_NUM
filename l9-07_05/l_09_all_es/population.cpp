...
Population::Population(int pop_size, Globe *globe, Random *rnd){
    ...
    if(globe->Get_N_cities() < 3){
        throw runtime_error("Not enough cities.");
    }
...
void Population::Order(){
    ...
    _best_individual=_individuals.at(0);//salva il migliore individuo
}
...
void Population::Mutate(Path & individual){
    ...
    //else if(which==3){
    //   individual=_best_individual; //L'individuo viene messo uguale al migliore di tutti
    //}
...
void Population::Print(const  char * filename)const{
    ...
    for (auto it = _individuals.begin(); it != _individuals.end(); ++it) {it->Printwlen(fout);}
